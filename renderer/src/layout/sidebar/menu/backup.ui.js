...
import { BackupAction, BackupController } from "./backup.controller";
import SubmitCancel from "../../../components/submit-cancel.ui";

export default function Backup(props) {
  const ctrl = useController(BackupController, props);

  if (!ctrl.authStore.state.sessionActive.value) return;

  const inProgress = ctrl.state.backupInProgress.value;

  return (
    <dialog id="backup-modal" class="modal">
      <form {...ctrl.form.props} id="backup-form" class="modal-box">
